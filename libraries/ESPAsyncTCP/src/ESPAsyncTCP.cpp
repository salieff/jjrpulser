...
err_t AsyncClient::_connected(void* pcb, err_t) {
...
err_t AsyncClient::_sent(tcp_pcb *, uint16_t len) {
#if ASYNC_TCP_SSL_ENABLED
...
err_t AsyncClient::_recv(tcp_pcb* pcb, pbuf* pb, err_t) {
    if(pb == NULL) {
...
err_t AsyncClient::_poll(tcp_pcb *) {
    // Close requested
    if(_close_pcb) {
        _close_pcb = false;
        _close();
...
// lWIP Callbacks
#if LWIP_VERSION_MAJOR == 1
void AsyncClient::_s_dns_found(const char *, ip_addr_t *ipaddr, void *arg) {
#else
void AsyncClient::_s_dns_found(const char *, const ip_addr *ipaddr, void *arg) {
#endif
...
err_t AsyncServer::_accept(tcp_pcb* pcb, err_t) {
    if(_connect_cb) {
#if ASYNC_TCP_SSL_ENABLED
